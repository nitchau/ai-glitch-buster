...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | Foundation | pnpm monorepo, Phaser 3 + TS + Vite scaffold, `_shared` package (forked quizData + profile SDK + theme), Vitest + Playwright + ESLint + tsc, GitHub Actions CI, Vercel project + monorepo settings, "Hello Glitch Guardians!" Phaser scene deployed to a preview URL | pending | - | - | - |
